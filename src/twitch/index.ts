...
/** The default Twitch embed provider (used by the custom element). Use for provider.play(), provider.pause(), provider.getPaused(). */
...
export function registerTwitchEmbed(): void {
  if (typeof customElements !== "undefined" && !customElements.get("twitch-embed")) {
    customElements.define("twitch-embed", createControllableEmbedElement(provider, "twitch-embed"));
  }
}
